...
const listToolDescription =
  "Lists available documents about the Global Payments API Platform, " +
...
server.tool(
  "list-sitemap-urls",
  "Lists files by reading the Global Payments Developer Portal sitemap at https://developer.globalpay.com/sitemap.xml.",
  async () => {
    ...
    return {
      content: urlList
        ...
        .filter(Boolean)
        .map((loc: string) => ({ type: "text", text: loc })),
    };
  }
);
...
server.tool(
  "fetch-sitemap-url",
  "Fetches the content of a remote Global Payments Developer Portal URL and returns its raw text.",
  { url: z.string().url() },
  async (extra) => {
...
const files = await listFilesRecursively(DOCS_DIR);
const docs = files.filter((file: string) => file.endsWith(".md") || file.endsWith(".yaml"))
  .map((file: string) => file.replace(DOCS_DIR, ""));

docs.forEach(file => {
  server.resource(file, `file://globalpayments${file}`, async (uri: URL) => {
    return {
      contents: [{
        uri: uri.href,
